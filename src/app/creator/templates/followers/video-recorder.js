...
export async function recordVideo(config) {
  ...
  if (config.image && config.image.category !== 'none') {
    let imageSrc = null;

    // Get image source based on category
    if (config.image.category === 'custom' && config.image.customImageUrl) {
      imageSrc = config.image.customImageUrl;
    } else if (config.image.selectedId) {
      const imageData = getImageById(config.image.selectedId);
...
async function exportMP4(canvas, ctx, confettiCanvas, fireConfetti, config, overlayImage) {
  ...
  if (config.type === 'dailyUpdate') {
    ...
    COUNTER_DURATION = (config.self.duration || 2) * 1000;
  }
  ...
  const { COUNTER_DURATION: templateCounterDuration, videoDuration } =
    calculateDuration(config, CONFETTI_DURATION);
